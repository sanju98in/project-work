...
public class GLMaintenance65461 extends GLMaintenanceWorkflow{
  
  @Test(groups = {"smoke"})
  public void  findDetailCode() throws InterruptedException, ParseException, IOException, URISyntaxException {
    ...
    //find detail code 
    searchText(testdataprop.getProperty("finddetailcode"));
    searchedTextAndValidateSearchMenuResult(orprop.getProperty("menusearchedtext"),testdataprop.getProperty("finddetailcode"));
    findCode(detailCode);
    callEvent(orprop.getProperty("button"),testdataprop.getProperty("submit"));
    validateGridIsAdded(orprop.getProperty("tablerevoverygroup"),detailCode.trim());
