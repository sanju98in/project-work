...
public class GenUtil extends TestBase{
  ...
  public static String getTheLatestDownloadedFile(String ext) {
    ...
    if (files.length > 0) {
      ...
      System.out.print("Total rows in the table is " + (rowTotal-1));
  ...
  public static String subtractDayOfDateFromCurrentDate(String minusNoOfDays) {
    // Get current date
    String DATE_FORMAT = "dd/MMMM/yyyy";
    DateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT);
    DateTimeFormatter dateFormat8 = DateTimeFormatter.ofPattern(DATE_FORMAT);
    ...
    dateFormat8.format(localDateTime).toString();
    // minus one
    localDateTime = localDateTime.minusDays(Integer.parseInt(minusNoOfDays));
  ...
  public static String findDayOfTheMonth(String additionalDate) {
    Calendar cal = Calendar.getInstance();
    int dayOfMonth = cal.get(Calendar.DAY_OF_MONTH);

    String.valueOf(dayOfMonth);
    int additionaDayOfMonth = dayOfMonth + Integer.parseInt(additionalDate) ;
    return Integer.toString(additionaDayOfMonth);
  }
  
  public static String getCurrentDateWithDateFormatter(String dateFormat) {
    // Get current date
    String DATE_FORMAT = "dd/MM/yyyy";
    DateFormat df = new SimpleDateFormat(DATE_FORMAT);
    DateTimeFormatter.ofPattern(DATE_FORMAT);
    Date currentDate = new Date();
    log.info("date : " + df.format(currentDate).toString());
